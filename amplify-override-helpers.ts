...
export const overrideDataSourceByFileName = (
    ...
    const [typeName, fieldName, slotName, slotPlacement] = resolverFileName.split('.');
    
    overrideDataSource(
        resources,
        (typeName as 'Mutation' | 'Query' | string),
        fieldName,
        (slotName as 'init' | 'preAuth' | 'auth' | 'postAuth' | 'preDataLoad' | 'postDataLoad' | 'finish' | 'preUpdate' | 'postUpdate' | 'preSubscribe'),
...
export const overrideDataSource = (
    ...
    slotName: 'init' | 'preAuth' | 'auth' | 'postAuth' | 'preDataLoad' | 'postDataLoad' | 'finish' | 'preUpdate' | 'postUpdate' | 'preSubscribe',
...
) =>
{
        ...
        )
    {
        ...
        if(
            ...
            && resources.models[targetModelName].appsyncFunctions[functionName].dataSourceName)
        {
            resources.models[targetModelName].appsyncFunctions[functionName].dataSourceName = newDataSourceName;
        }
        else
        {
            throw Error(    
